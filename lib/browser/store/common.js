            ...
            IotaSDK.getValidAddresses(newCurWallet).then(({ addressList, outputIds, smrOutputIds, requestAddress }) => {
                ...
                if (IotaSDK.checkWeb3Node(newCurWallet.nodeId)) {
                    ...
                    const batchRequest = async (walletInfo, cursor, requestId) => {
                        ...
                        const key = `${walletInfo.nodeId}.${walletInfo.address}.common.activityList`
                        ...
                        if (hisList.length > 0) {
                            ...
                            await Base.setLocalData(key, hisDic[requestAddress])
                            if (requestId !== requestHisId) {
                                return
                            }
                    ...
                    const key = `${newCurWallet.nodeId}.${newCurWallet.address}.common.activityList`
                    Base.getLocalData(key).then(async (list) => {
                        hisDic[newCurWallet.address] = list || []
