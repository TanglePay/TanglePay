...
export const useUpdateBalance = () => {
    ...
    const updateBalance = async (address, list, curNodeId) => {
        ...
        const newList = list.map(
            ({ available, realAvailable, realBalance, token, contract, balance, decimal, isSMRToken, tokenId }) => {
    ...
    const updateHisList = async (activityList, { address, nodeId }) => {
        ...
        if (isWeb3) {
            ...
            tokenIds.forEach((e, i) => {
                let { decimals, symbol } = IotaSDK.handleFoundry(foundryList[i])
                symbol = (symbol || '').toLocaleUpperCase()
                tokenDic[e] = { decimals, symbol }
            })
            // token info end
