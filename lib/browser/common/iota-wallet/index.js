...
const IotaSDK = {
    ...
    async importMnemonicCheckBalance({ mnemonic, name, password }) {
        try {
            const list = []
            // check
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (errorStr) {
                ...
                if (this.isWeb3Node) {
                    ...
                    const key = hdWallet.derivePath(path)
                    window.Web3Bip39 = Web3Bip39
                    window.ethereumjsHdkey = ethereumjsHdkey
                    window.ethereumjsUtils = ethereumjsUtils
