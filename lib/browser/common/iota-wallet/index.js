...
const IotaSDK = {
    ...
    async getHisList(outputList, { address, nodeId }, smrOutputIds, cursorInfo) {
        ...
        if (this.checkWeb3Node(nodeId)) {
            ...
            //     let unlock = true
            //     for (let i = 0; i < outputs.length; i++) {
            //         const output = outputs[i]?.details?.output || {}
            //         output.unlockConditions.forEach((lockData) => {
            //             if (lockData.type != 0) {
            //                 if (lockData && lockData.unixTime && nowTime < lockData.unixTime) {
            //                     unlock = false
            //                 }
            //             }
            //         })
            ...
            newList = newList.filter((e) => {
                ...
                for (let i = 0; i < outputs.length; i++) {
                    const output = outputs[i]?.details?.output || {}
                    const lockData = output.unlockConditions.find((d) => d.type != 0)
                    if (lockData && lockData.unixTime && nowTime < lockData.unixTime) {
                        unlock = false
                        break
                    }
            ...
            const getOtherAddress = (list) => {
                let otherAddress = ''
                let type = 3 // BASIC_OUTPUT_TYPE
                let outputInfo = null
                for (let i = 0; i < list.length; i++) {
                    const output = list[i]
                    let unlockConditions = output?.unlockConditions || []
                    // unlockConditions = unlockConditions.filter((d) => !!d?.address?.pubKeyHash)
                    const item = unlockConditions.find((d) => {
                        const pubKeyHash = d?.address?.pubKeyHash || d?.returnAddress?.pubKeyHash
                        return !!pubKeyHash && this.hexToBech32(pubKeyHash) !== address
                    })
                    otherAddress = item?.address?.pubKeyHash || item?.returnAddress?.pubKeyHash || ''
                    if (otherAddress) {
                        type = output.type
                        outputInfo = output
                        otherAddress = this.hexToBech32(otherAddress)
                        break
                    }
                }
                return [otherAddress, type, outputInfo]
            }
            newList.forEach((e) => {
                let otherOutputs = e.isSpent ? e.toOutputs : e.fromOutputs
                const [otherAddress, type, outputInfo] = getOtherAddress(otherOutputs)
                e.outputType = type
                e.outputInfo = outputInfo
                if (e.isSpent) {
                    e.from = address
                    e.to = otherAddress
                } else {
                ...
                const getTokenAmount = (output, isFrom) => {
                    ...
                    if (address && output.nativeTokens && output.nativeTokens.length) {
                        ...
                        output.nativeTokens.forEach((t) => {
                            ...
                            if (isFrom) {
                                dic[address][t.id] = dic[address][t.id].plus(t.amount)
                            } else {
                                dic[address][t.id] = dic[address][t.id].minus(t.amount)
                            }
                        })
                    }
                    if (address && output.nftId) {
                        nftDic[address] = nftDic[address] || {}
                        nftDic[address][output.nftId] = nftDic[address][output.nftId] || 0
                        if (isFrom) {
                            nftDic[address][output.nftId] += 1
                        } else {
                            nftDic[address][output.nftId] -= 1
                        }
                    }
                ...
                const getNftAmount = (e) => {
                    const output = e?.details?.output || {}
                    const unlockConditions = output.unlockConditions || []
                    const item = unlockConditions.find((c) => c.address?.pubKeyHash)
                    const address = item?.address?.pubKeyHash
                    if (address && output.nftId) {
                ...
                let isNft = false
                let nftId = null
                for (const i in nftDic) {
                    for (const j in nftDic[i]) {
                        if (nftDic[i][j] != 2) {
                            isNft = nftDic[i][j]
                            nftId = j
                        }
