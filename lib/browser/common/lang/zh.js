const ZH = {
    account: {
        needPinToTurnoffPassword: '您需要設置PIN碼才能關閉支付密碼',
        typeYourPin: '輸入您的PIN碼',
        welcomeBack: '歡迎回來!',
        invalidPassword: '無效的密碼',
        passwordDisabled: '密碼已禁用',
        enterCurrentPassword: '輸入當前密碼',
        passwordEnabled: '密碼已啟用',
        walletPasswordTitle: '啟用付款密碼',
        disableWalletPassword: '停用付款密碼',
        enterNewPassword: '輸入新密碼',
        retypeNewPassword: '重新輸入新密碼',
        passwordMismatch: '密碼不匹配',
        confirm: '確認',
        toggleWalletPassword: '支付密碼',
        pinResetSuccess: 'PIN 碼重置成功',
        pinSetSuccess: 'PIN 碼設置成功',
        invalidOldPin: '舊 PIN 碼無效',
        pinMismatch: '新 PIN 碼與重新輸入的 PIN 碼不符',
        setPinTitle: '設定 PIN 碼',
        newPin: '新 PIN 碼',
        enterNewPin: '輸入您的新 PIN 碼',
        retypePin: '重新輸入 PIN 碼',
        retypeNewPin: '重新輸入您的新 PIN 碼',
        setPinButton: '設定 PIN 碼',
        resetPinTitle: '重置 PIN 碼',
        oldPin: '舊 PIN 碼',
        enterOldPin: '輸入您的舊 PIN 碼',
        resetPinButton: '重置 PIN 碼',
        title: '##TanglePay## Wallet',
        subTitle: `IOTA Tangle Technology\nGoing Beyond Blockchain`,
        create: '創建新賬號',
        hasWallet: '登陸賬號',
        intoTitle1: '助記詞導入',
        intoTitle2: '備份文件導入',
        mnemonicTips: '請填入助記詞，詞與詞間用空格隔開',
        intoSelectFile: '選擇Stronghold文件',
        intoName: '設置賬號名稱',
        intoNameTips: '請填入名稱',
        intoPassword: '設置密碼',
        intoPin: '設定PIN碼',
        intoPasswordTips: '8到20位數字和字母的組合',
        intoPinTips: '8到20位數字和字母的組合',
        pinTryLeft: '剩餘 ${left} 次嘗試',
        pinCanNotTryUntil: '無法在 ${time} 之前嘗試',
        intoRePasswordTips: '請再次輸入密碼',
        intoRePin: '請重複輸入PIN碼。',
        intoAgree: '我已閱讀並理解 ##Terms of Service## 和 ##Privacy Policy##',
        term: 'Terms of Service',
        policy: 'Privacy Policy',
        intoBtn: '導入賬號',
        intoFilePassword: '密碼',
        intoFilePasswordTips: '請輸入Stronhold文件密碼',

        createTitle: '創建賬號',
        passwordOptional: '設置密碼',

        backupTitle: '備份賬號',
        backupTips: '備份提示',
        backupTipsContent: `獲得助記詞等於擁有所有資產。

一旦助記詞丟失，資產將無法恢復。請妥善保管您的助記詞，並將其與網絡隔離。
        
請勿通過網絡分享或儲存助記詞，如截圖、郵件、社交媒體等`,
        next: '下一步',

        backupScreenshoptTitle: '請勿截屏',
        backupScreenshoptTips: '請勿截屏助記詞，該操作可能帶來由第三方獲取助記詞而引起的資產損失',
        backupScreenshoptBtn: '我已理解',

        mnemonicTitle: '備份助記詞',
        mnemonicAggre: '我已將我的助記詞安全存放並將其與網絡隔離。',
        gotoWallet: '前往賬號',

        mnemonicSubTitle: '助記詞可用於找回賬號，請務必按順序妥善記錄並保管',
        mnemonicPhraseTips1: '助記詞一旦丟失則資產無法找回，請務必將記錄好的助記詞在線下妥善保管',
        mnemonicPhraseTips2: '請勿在線上存儲或分享助記詞，如截屏、郵件、社交媒體等',
        mnemonicBtn: '已完成備份，現在驗證',
        mnemonicExp: '助記詞示例',

        registerSucc: `允許 Tanglepay 收集您的數據並幫助我們持續改善產品的用戶體驗。


<##TanglePay 將##>
始終允許您通過「設置」退出 

發送匿名點擊和頁面查看事件

發送城市、國家、地區信息（不包含具體地址）


<##TanglePay 永遠不會##>
收集您的密碼、地址、交易、余額或任何個人信息

收集您的 IP 地址

出售您的數據以獲取利潤

此數據處理滿足歐盟法規 2016/679 通用數據保護條例的要求，因此是匿名的。有關我們隱私保護實踐的更多信息，請參閱我們的隱私政策`,
        start: '開始',

        mnemonicError: '助記詞長度為{len}個單詞',

        unopen: '暫未開放',
        testBackup: '助記詞驗證',
        checkPin: '您輸入的PIN碼不符，請再試一次。',
        checkPasswrod: '兩次輸入的密碼不一致，請重新輸入',
        mnemonicWordError: '助記詞中包含無法識別的單詞{word}',
        mnemonicOtherError: '助記詞錯誤，請重新輸入',

        dearFam: '##TanglePay## Wallet',
        betaReady: `IOTA Tangle Technology
Going Beyond Blockchain`,
        changeTips: '請選擇您將要首先使用的網絡。您可以在登陸錢包後隨時添加其他網絡。',
        devnet: 'IOTA 測試網',
        mainnet: 'IOTA 主網',
        evmnet: 'IOTA EVM',
        selectNode: '選擇網絡',
        exportKey: '導出私鑰',
        exportMnemonic: '導出助記詞',
        showKey: '顯示私鑰',
        showKeyInputPassword: '請輸入 {name} 的密碼',
        copyKeyTips: '這是您的私鑰（點擊復製）',
        showKeyTips: '提示：請永遠不要泄漏您的私鑰，任何人擁有您的私鑰就可以控製您的賬戶。',
        showKeyMnemonic: '顯示助記詞',
        copyKeyTipsMnemonic: '這是您的助記詞（點擊復製）',
        showKeyTipsMnemonic: '提示：請永遠不要泄漏您的助記詞，任何人擁有您的助記詞就可以控製您的賬戶。',
        done: '已完成',
        viewInExplorer: '查看explorer',
        or: '或',
        importDuplicate: '該賬號已完成導入',
        privateKeyImport: '私鑰導入',
        inputPrivateKey: '請填入私鑰',
        removeTitle: '移除賬號',
        removeTips: `該賬號將會從您的設備中移除。請確保您已經將助記詞或私鑰妥善保管。`,
        removeName: '賬戶名',
        removeAddress: '地址',
        nevermind: '取消',
        remove: '移除',

        walletDetail: '賬號詳情',
        seedAddresses: '該seed下的地址',
        exportExcel: '導出',
        address: '地址',
        outputNum: 'Output數',
        iotaNum: '金額 MIOTA',
        totalNum: '總額',
        outputCollect: 'output歸集',
        collectTips: `說明：Output合併會將您的資產從多個地址和 UTXO（未使用的交易輸出）聚合到您的主地址上的統一 UTXO 中，這將提高後續活動的性能，包括但不限於獲得餘額和轉幣。`,
        pendingNum: '待處理output條數',
        processedNum: '已處理的條數',
        collectTermination: '終止',
        collectSuccTips: 'Output歸集已完成',
        hardwareWallet: '連接硬件錢包',
        connectLedger: '連接Ledger',
        lederImport: 'Ledger導入'
    },
    assets: {
        sentTo: '您将进行支付',
        myAssets: '我的資產',
        send: '發送',
        receive: '收取',
        assets: '資產',
        activity: '操作記錄',
        search: '搜索',
        myWallets: '我的賬號',
        addWallets: '添加賬號',
        copied: '已復製',
        currency: '幣種',
        receiver: '收款地址',
        receiverTips: '添加或粘貼地址',
        amount: '數量',
        amountTips: '添加轉賬數量',
        balance: '余額',
        password: '密碼',
        passwordTips: '輸入密碼',
        confirm: '確認',

        scanQRcode: '通過掃碼收款',
        copy: '復製',
        share: '分享',

        addAssets: '添加資產',
        addAssetsTips: '輸入代幣名稱或合約地址',

        scanTitle: '掃碼',
        scanPermissionsTitle: '申請攝像頭權限',
        scanPermissionsTips: '應用想使用你的攝像頭掃描二維碼',
        scanTips: '將二維碼放入框內，即可自動掃描',
        scanError: '未開啟攝像頭權限，無法掃描二維碼',

        noWallet: '您還沒有添加賬號哦',

        sendSucc: '發送成功',
        sendSuccRestake: '發送成功',
        sendError: '發送失敗，請檢查收取地址是否正確',
        passwordError: '賬號密碼錯誤',
        balanceError: '余額不足',
        balanceStakeError: '資金不足。請先unstake。',
        sendErrorInsufficient: `錯誤：資金不足，無法進行交易。
發送{token}數量：{amount} 
預計存儲押金：{deposit} {unit} 
可用{token}餘額：{balance1} 
可用 {platformToken} 餘額：{balance2} {unit}
鎖定的 {platformToken} 餘額：{balance3} {unit}`,

        requestAssets: '正在獲取資產數據',
        requestHis: '正在獲取歷史交易記錄',

        receivedSucc: '成功接收{token} {num} {unit}',
        album: '相冊',
        readFail: '二維碼識別失敗',

        residueBelow1Tips: '剩余數量不能是小於1的小數，請重新輸入金額。',
        sendBelow1Tips: '發送金額不能少於1 Mi，請重新輸入金額。',
        evmGasNotSufficient: '地址中 {token} 余额不足以支付Gas Fee，请充值',
        sendSuccRestakeTips: '交易成功。',
        restakeSuccTips: 'Restaking 成功',
        restakeFailTips: 'Restaking 失败',
        tradingList: '交易列表',
        tradingFrom: '從',
        tradingTitle: '轉賬',
        acceptTitle: '接受轉賬',
        storageDeposit: '存儲押金',
        standard: '標準',
        tokenID: '代幣ID',
        dismissTips: '請確認是否暫不接收，資產將在到期後退回發送賬戶。',
        acceptSucc: '領取成功',
        unlockError: '請確認地址中有足夠{token}用於領取代幣。',
        locked: '已鎖定',
        unlockTime: '解鎖時間',
        tokenName: '名稱',
        tokenDetail: '代幣詳細信息',
        nftInfo: 'NFT信息',
        nftDetail: 'NFT詳細信息',
        collectionID: 'NFT收藏集ID',
        nftProperties: '屬性',
        importToken: '導入代幣',
        tokenContractAddress: '代幣合約地址',
        inputContractAddress: '輸入或粘貼地址',
        tokenSymbol: '代幣符號',
        inputTokenSymbol: '輸入或粘貼代幣符號',
        tokenDecimal: '代幣小數點個數',
        inputTokenDecimal: '輸入或粘貼小數點個數',
        inputRightContract: '請輸入有效的合約地址',
        customTokens: '自定義代幣',
        searchToken: '搜索代幣',
        importBtn: '導入',
        estimateGasFee: '估計手續費',
        editPriority: '編輯優先權',
        gasFee: '手續費',
        gasPrice: 'Gas Price',
        gasLimit: '手續費限額',
        maxFee: '最大費用',
        edit: '編輯',
        getAssetsFail: '資產同步失敗，請下拉刷新',
        getActivityFail: '交易記錄同步失敗，請下拉刷新',
        sendConfirmation: '確認劃轉',
        sendConfirmationTips: '請確認本次交易',
        sendConfirmation: '發送確認',
        sendConfirmationTips: '請確認發送此交易',
        sendInsufficientNfts: '找不到NFT。',
        sendInsufficientNativeToken: '{token} 不足。',
        sendInsufficientCash: '{cash}不足。',
        sendAmountTooSmall: '轉帳金額不能低於 {amount}。',
        sendStorageDepositNotEnough: '儲存存款不足。',
        sendInsufficientCashForRemainderDeposit: '剩餘的 {amount} 不足以支付儲存存款。'
    },
    user: {
        me: '個人',
        manageWallets: '管理賬號',
        setting: '設置',
        network: '節點',
        privacy: '隱私',
        privacyTips: '參加TangleMetrics',
        aboutUs: '關於我們',
        language: '語言',
        biometrics: '生物認證',
        enableBiometrics: '生物辨識技術',
        bioVerification: '生物辨識',
        noPrompt: '不再提示',
        biometricsSucc: '生物辨識成功',
        biometricsFailed: '生物辨識失敗',
        biometriceDialog: '生物辨識開啟後，每筆交易將不再需要手動輸入密碼。',
        goSetting: '前往設置',
        manage: '管理',
        backupWallet: '備份賬號',
        backupWalletTips: `導出到Stronghold文件 - 
一份您的賬號和最新交易歷史的完整加密備份`,
        export: '導出',
        resetPassword: '重置密碼',
        old: '舊的',
        oldTips: '請輸入您舊的密碼',
        new: '新的',
        newTips: '請輸入您新的密碼',
        repeatPassword: '再次輸入密碼',
        exportNewFile: '導出一份Stronghold文件',

        nodeError: '節點未同步',
        passwordError: '舊密碼輸入錯誤',
        passwordSucc: '密碼重置成功',

        curVersion: '當前版本 ',
        versionUpdate: '版本更新',
        versionNew: '最新',
        website: '官方網站',
        telegramGroup: 'Telegram Group',
        discord: 'Discord',
        groupEmail: '郵箱',

        latestVersion: '當前已是最新版本'
    },
    staking: {
        title: 'Staking',
        preStake: 'Pre-stake',
        his: 'Staking記錄',
        stake: 'Stake',
        airdrops: 'Airdrops',
        startAt: '開始於',
        amount: '數量',
        available: '可用',
        availableToStake: '可以stake',
        estimatedReceived: '預期已獲得收益',
        airdropsList: 'Airdrops列表',
        add: '添加',
        unstake: 'Unstake',
        staked: 'Staked',
        enterAmount: '添加數量',
        max: '最大化',
        password: '密碼',
        confirm: '確認',
        addAirdrop: '添加 {name} Airdrop',
        token: '代幣',
        viewAll: '查看全部',
        addAmount: '添加數量',
        addAirdrop: '添加 Airdrop',
        noAvailableTips: '您的賬戶沒有可用IOTA，請轉入IOTA參與空投。',
        stakeBelow1Tips: '數量不能少於1Mi，請重新輸入。',
        soon: '即將開始',
        limitAmount: '至少需要 {num} MIOTA 才可參與 {token} staking.',
        restake: '自動 Restake'
    },
    apps: {
        title: 'Dapps',
        sendFrom: '來自',
        sendFor: '用於購買',
        send: `您有一個支付申請#merchant##item_desc#，將支付到以下地址，
#address#
支付金額為 #amount# #unit#。
請填寫密碼完成支付。`,
        sign: `來自 #merchant# 的簽名請求
您在簽名：
#content#`,
        execute: '確認',
        cancel: '取消',
        refresh: '刷新',
        reject: '拒絕',
        signLabel: 'TanglePay 簽名',
        connect: `#origin#
連接
#address#
允許網站: 查看地址, 賬號余額
`,
        ConnectBtn: '連接',
        approve: `通過授予許可，您允許以下合約訪問您的資金。
#address#
權限: #unit#
`,
        contractFunc: `function: #abiFunc#`
    },
    nft: {
        collectibles: 'NFTs',
        zeroTips: '您暫時還沒有{name}',
        clearCache: '清理緩存',
        goBuy: '去購買',
        permissions: '請允許TanglePay保存NFT圖片到您的相冊',
        saved: '保存成功',
        continueCombine: '繼續合成',
        nftAdd: '添加',
        totalNum: '共{num}項',
        selectHero: '選擇 Hero Tier'
    },
    discover: {
        title: '發現',
        buyIota: '購買IOTA',
        addressCopy: '地址已復製'
    },
    shimmer: {
        network: 'Shimmer 主網',
        claimStakingReward: '獲取Staking收益',
        chooseAWallet: '選擇一個賬號來領取##SMR Staking獎勵##',
        claimStakingRewards: '獲取SMR Staking收益',
        importTips: '如果你要獲取收益的IOTA賬號不在列表內，請先在Tanglepay中 ##導入IOTA賬號##.',
        claim: '獲取',
        claimingFailed: '獲取失敗',
        claimingFailedTips: '您的IOTA賬號##{name} {address}##中沒有可以Claim的Shimmer Staking Rewards',
        understand: '我已知曉',
        smrClaimStakingReward: 'SMR Staking 獎勵已領取',
        smrAmount: 'SMR數量：',
        createTips: '新創建的Shimmer賬號助記詞與初始密碼與您的IOTA賬號 ##{name} {address}##一致',
        createSuccTips: '為了您的資產安全，建議您修改賬號密碼或者將資產轉移到全新的Shimmer賬號',
        tradingList: '交易列表',
        conditionsClaim: '獲取條件：',
        transaction: '交易',
        acceptTransaction: '接受交易',
        accept: '接受',
        dismiss: '撤銷',
        sendFailTips: '交易正在讀取中。此時Shimmer節點同步比較緩慢。',
        sendCancel: '取消交易',
        sendTips: '我們將在近期支持Shimmer Beta網絡的轉出功能。'
    }
}

export default ZH
