...
const ConsolidationStopThresInputsNums = 120;
...
const finishingCtx = (ctx) => {
    ...
    if (ctx.outputSMRBalance.lt(ctx.inputSMRBalance)) {
        ...
        if (BigNumber(cashOutput.amount).lt(BigNumber(cashOutputDeposit.toString()))) {
            ...
            const errStr = I18n.t('assets.sendInsufficientCashForRemainderDeposit').replace(/{amount}/g, cashOutput.amount+' '+(isIotaStardust ? 'MICRO' : 'GLOW'))
            throw new Error(errStr)
        }
