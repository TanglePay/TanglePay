...
const IotaSDK = {
    ...
    async send(fromInfo, toAddress, sendAmount, ext) {
        ...
        if (this.checkWeb3Node(nodeId)) {
            ...
            // awaitTime: await ms
            const { residue, awaitStake, awaitTime = 10000 } = ext || {}
            if (this.curNode.id === this.IOTA_NODE_ID && !isNoRestake && residue > 0) {
