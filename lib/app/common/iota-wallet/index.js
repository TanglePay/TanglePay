...
const IotaSDK = {
    ...
    async getTransport() {
        if (!Base.transport) {
            console.log(TransportWebHID, TransportWebUSB, TransportWebBLE)
            ...
            console.log(hasHID, hasUSB, hasBLE, Base.isBrowser)
            if (Base.isBrowser) {
                if (hasHID) {
    ...
    async send(fromInfo, toAddress, sendAmount, ext) {
        ...
        if (this.checkWeb3Node(nodeId)) {
            ...
            if (contract || taggedData) {
                ...
                console.log(signData, '______________')
