...
const IotaSDK = {
    ...
    async send(fromInfo, toAddress, sendAmount, ext) {
        ...
        if (this.checkWeb3Node(nodeId)) {
            ...
            if (contract || taggedData) {
                ...
                const contractGasLimit = ext.gas || contractInfo?.gasLimit
                const { gasLimit } = await this.getGasLimit(contractGasLimit, address, taggedData ? sendAmount : 0)
                if (gasLimit === -1) {
                    return Base.globalToast.error(
    ...
    /**************** Nft start *******************/
    async getNfts(addressList, origins) {
    ...
    getAbiParams(contract, hex) {
        ...
        if (web3Contract) {
            ...
            if (!item) {
                var LpRouterAbi = require('../abi/LpRouter.json')
                var PoolAbi = require('../abi/Pool.json')
                var SwapRouterAbi = require('../abi/SwapRouter.json')
                var FactoryAbi = require('../abi/Factory.json')
                new this.client.eth.Contract(LpRouterAbi, contract)
                new this.client.eth.Contract(PoolAbi, contract)
                new this.client.eth.Contract(SwapRouterAbi, contract)
                new this.client.eth.Contract(FactoryAbi, contract)
                item = [...LpRouterAbi, ...PoolAbi, ...SwapRouterAbi, FactoryAbi].find(
                    (e) => e.signature === functionSign
                )
            }
            if (item) {
