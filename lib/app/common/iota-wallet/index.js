...
const IotaSDK = {
    ...
    async getGasLimit(configLimit, address, sendAmount) {
        ...
        gasPrice = new BigNumber(gasPrice).times(2).integerValue(1)
    ...
    async send(fromInfo, toAddress, sendAmount, ext) {
        ...
        if (this.checkWeb3Node(nodeId)) {
            ...
            if (contract || taggedData) {
                ...
                const { gasLimit, gasPrice } = await this.getGasLimit(
                    contractGasLimit,
                    address,
                ...
                try {
                    ...
                    if (/gas limit/.test(error.toString()) && this._ethReSend <= 10) {
                        this._ethReSend++
