import { useContext, useEffect, useState } from 'react'
import { IotaSDK, API_URL, Base } from '../common'
import { StoreContext } from './context'
import { useGetNodeWallet } from './common'
import _uniqWith from 'lodash/uniqWith'
import _isEqual from 'lodash/isEqual'
import _get from 'lodash/get'
export const initState = {
    events: [],
    config: { airdrops: [], filter: [], rewards: {} },
    historyList: [],
    statedTokens: [],
    statedAmount: 0,
    stakedRewards: {},
    forceRequest: 0, //force data sync indicator
    needRestake: false,
    curStatedRewards: {},
    participationEvents: {}
}
export const reducer = (state, action) => {
    let { type, data } = action
    switch (type) {
        case 'historyList': {
            const list = data || []
            let statedTokens = []
            let statedAmount = 0
            let eventIds = {}
            const lastData = list[list.length - 1] || {}
            let lastAddress = lastData.address || ''
            if (lastData.type !== 3) {
                statedAmount = lastData.amount || 0
            }
            list.forEach((e) => {
                const { tokens } = e
                tokens.forEach((d) => {
                    const { eventId } = d
                    if (!eventIds[eventId]) {
                        statedTokens.push({
                            ...d
                        })
                    }
                    eventIds[eventId] = 1
                })
            })
            statedTokens = statedTokens.map((e) => {
                e.address = lastAddress
                return { ...e }
            })
            return { ...state, [type]: list, statedTokens, statedAmount: state.needRestake ? 0 : statedAmount }
        }
        case 'needRestake': {
            const obj = { [type]: data }
            if (data) {
                obj.statedAmount = 0
            }
            return { ...state, ...obj }
        }
        case 'participationEvents':
            Base.setLocalData('staking.participationEvents', data)
            return { ...state, [type]: data }
    }
    return { ...state, [type]: data }
}
export const useGetEventsConfig = () => {
    const { _, dispatch } = useContext(StoreContext)
    useEffect(() => {
        fetch(`${API_URL}/events.json?v=${new Date().getTime()}`)
            .then((res) => res.json())
            .then((res) => {
                const airdrops = res?.airdrops || []
                const filter = res?.filter || []
                const rewards = res?.rewards || {}
                dispatch({
                    type: 'staking.config',
                    data: { airdrops, filter, rewards }
                })
                Base.setLocalData('local.events.json', { airdrops, filter, rewards })
            })
            .catch((err) => {
                console.log(err)
                Base.getLocalData('local.events.json').then((localRes) => {
                    if (localRes) {
                        dispatch({
                            type: 'staking.config',
                            data: localRes
                        })
                    }
                })
            })
    }, [])
}

export const useGetParticipationEvents = () => {
    // const [curWallet] = useGetNodeWallet();
    const { store, dispatch } = useContext(StoreContext)
    const curNodeId = _get(store, 'common.curNodeId')
    const { filter } = _get(store, 'staking.config')
    const [eventInfo, setInfo] = useState({})
    useEffect(() => {
        const getData = async () => {
            if (!curNodeId || !IotaSDK.hasStake(curNodeId)) {
                return
            }
            let datas = await IotaSDK.getParticipationEvents()
            datas = datas.filter((e) => !filter.includes(e.id))
            const nowTime = parseInt(new Date().getTime() / 1000)
            datas.sort((a, b) => b.milestoneIndexCommence - a.milestoneIndexCommence)
            const unPreList = datas.filter((e) => e.commenceTime > nowTime)
            if (unPreList.length > 1) {
                datas = datas.slice(unPreList.length - 1)
            }
            datas.sort((a, b) => a.milestoneIndexStart - b.milestoneIndexStart)
            const upcomingList = []
            const commencingList = []
            const endedList = []
            let info = {
                status: 0,
                list: [],
                upcomingList,
                commencingList,
                endedList
            }
            if (datas.length > 0) {
                // status：0->Ended  1->Upcoming ，2->Commencing
                datas.forEach((e) => {
                    const { startTime, endTime } = e
                    if (nowTime < startTime) {
                        upcomingList.push({ ...e })
                    } else if (nowTime >= startTime && nowTime < endTime) {
                        commencingList.push({ ...e })
                    } else {
                        endedList.push({ ...e })
                    }
                })
                let status = 0
                if (upcomingList.length > 0) {
                    status = 1
                } else if (commencingList.length > 0) {
                    status = 2
                }
                info = {
                    status,
                    list: datas,
                    upcomingList,
                    commencingList,
                    endedList
                }
            }
            dispatch({
                type: 'staking.participationEvents',
                data: info
            })
            setInfo(info)
        }
        if (IotaSDK.hasStake(curNodeId)) {
            getData()
        }
    }, [curNodeId, JSON.stringify(filter)])
    return [eventInfo, setInfo, filter]
}

export const useGetRewards = (curWallet, istTurn = true) => {
    const { store, dispatch } = useContext(StoreContext)
    const [rewards, setRewards] = useState({})
    const validAddresses = _get(store, 'common.validAddresses')
    const eventInfo = _get(store, 'staking.participationEvents')
    const endedList = eventInfo?.endedList || []
    useEffect(() => {
        let timeHandler = null
        if (!curWallet.address) {
            setRewards({})
        } else {
            const requestData = async () => {
                IotaSDK.getAddressListRewards(validAddresses)
                    .then((dic) => {
                        if (JSON.stringify(rewards) !== JSON.stringify(dic)) {
                            setRewards(dic)
                            dispatch({
                                type: 'staking.stakedRewards',
                                data: dic
                            })
                            Base.setLocalData(`${curWallet.nodeId}.${curWallet.address}.staking.stakedRewards`, dic)
                            const curDic = {}
                            for (const i in dic) {
                                if (!endedList.find((e) => e.id == i)) {
                                    curDic[i] = { ...dic[i] }
                                }
                            }
                            dispatch({
                                type: 'staking.curStatedRewards',
                                data: curDic
                            })
                        }
                    })
                    .catch(() => {
                        setRewards({})
                        dispatch({
                            type: 'staking.stakedRewards',
                            data: {}
                        })
                        dispatch({
                            type: 'staking.curStatedRewards',
                            data: {}
                        })
                    })
            }
            requestData()
            if (istTurn) {
                timeHandler = setInterval(requestData, 10000)
            }
        }
        return () => {
            clearInterval(timeHandler)
        }
    }, [curWallet.address + curWallet.nodeId, JSON.stringify(validAddresses), JSON.stringify(endedList)])
    return rewards
}
