...
const IotaSDK = {
    ...
    async getBalance0Address({ seed, password }) {
        const accountState = {
            accountIndex: 0,
            addressIndex: 0,
            isInternal: false
    ...
    async getParticipationEvents() {
        ...
        events.forEach((e) => {
            const { milestoneIndexStart, milestoneIndexEnd, payload } = e
            ...
            list.push({ ...e, limit, startTime: turnTime(milestoneIndexStart), endTime: turnTime(milestoneIndexEnd) })
        })
        return list
    },
    // type：1-》stake  2-》add amount  3-》unstake 4-》add airdrop
    async handleStake({ wallet, tokens, amount, type }) {
        console.log(wallet, tokens, amount, type)
        try {
    ...
    async sendParticipateMessage({ wallet, tokens, amount, type }) {
        type = parseInt(type)
        amount = parseFloat(amount * this.IOTA_MI)
        const { seed, password } = wallet
        const address = await this.getBalance0Address({ seed, password })
        const baseSeed = this.getSeed(seed, password)
        let datas = []
