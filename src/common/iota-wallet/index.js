    ...
    async getHisList(outputList, { address, nodeId }) {
        ...
        if (this.checkWeb3Node(nodeId)) {
            ...
            if (this.checkSMR(nodeId)) {
                let outputDatas = await Promise.all(outputList.map((e) => this.outputData(e)))
                const blockDatas = await Promise.all(outputDatas.map((e) => this.blockData(e.metadata.blockId)))
